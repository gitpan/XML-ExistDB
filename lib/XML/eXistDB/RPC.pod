...
just an one-on-one implementation, but some methods are smarter.
...
collects a list of XXX names.  All Java typical C<get> method names
were removed in favor of better named alternatives: sometimes C<list>,
sometimes C<describe>, often something completely different. Class
attribute getters and setters naming should not be used in interfaces
(and very not-Perl).
...
RPC is a network protocol. Just like operating calls: you shall always
check the return status of each call!  Of course, this module could simply
ignore the existence of fault conditions, to provide a much simpler
...
All method return a LIST, where the first scalar is a return code (RC).
When that code is '0', all went well.  Otherwise, the code represent the
...
returned with XML::eXistDB::RPC subroutine getCollectionDescr, excluding details about
documents.
...
is more consistent than the official API name XML::eXistDB::RPC subroutine remove.

=back

$obj-E<gt>B<typeOfResource>(RESOURCE)
...
whether L<upload()|XML::eXistDB::RPC/"Please avoid: uploading documents">, uploadCompressed, or L<parse()|XML::eXistDB::RPC/"Please avoid: uploading documents"> is used to
...
Replaces XML::eXistDB::RPC subroutine getHits.
...
Please use describeCollection() with option C<< documents = 0 >>.
...
STYLE must be present but may be C<undef>.
...
Please use L<retrieveDocumentNode()|XML::eXistDB::RPC/"Simple node queries"> or L<retrieveResult()|XML::eXistDB::RPC/"Query returns resultset">.
...
Please use L<retrieveResult()|XML::eXistDB::RPC/"Query returns resultset"> or L<retrieveDocumentNode()|XML::eXistDB::RPC/"Simple node queries">.
...
Please use L<retrieveResults()|XML::eXistDB::RPC/"Query returns resultset">.
...
=head2 Methods with unknown purpose

For all the next methods, it is unclear what they do, or some it their
required parameters behave. Please help me documenting them, and I will
implement (and probably rename) them.
